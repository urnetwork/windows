...
FrameworkElement MakeIdenticonBadge(IdenticonCache& cache, const ProviderIdentityRow& identity,
                                    int displaySize) {
  // qualified: pch.h pulls in windows.h, whose GDI ::Rectangle makes the bare
  // name ambiguous (the same reason the other files alias ShapeEllipse)
