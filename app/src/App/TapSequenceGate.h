...
// connected dot, each within two seconds of the last, play the Pro
// celebration). A gap longer than the window, or a completed sequence, starts
// the count over. Pure logic, no clock of its own: the caller passes the time
// of each tap (the same helper the Linux app carries, with its unit tests, in
// app/src/TapSequenceGate.hpp).
//
...
class TapSequenceGate {
 public:
  explicit TapSequenceGate(int count = 5, int64_t windowMs = 2000)
