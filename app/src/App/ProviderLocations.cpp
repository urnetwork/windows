...
std::vector<ProviderLocationRow> WheelOrder(const std::vector<ProviderLocationRow>& rows) {
  std::vector<ProviderLocationRow> wheel;
  wheel.reserve(rows.size());
  for (const auto& row : rows) {
    if (row.Plottable()) wheel.push_back(row);
  }
  // stable so providers sharing a longitude keep the sdk's duration order
  std::stable_sort(wheel.begin(), wheel.end(),
                   [](const ProviderLocationRow& a, const ProviderLocationRow& b) {
                     return a.lon < b.lon;
                   });
  return wheel;
}
