...
StackPanel Onboarding::BuildReferral() {
  ...
  page.Children().Append(referralPanelHost_);
...
void Onboarding::ApplyReferral() {
  ...
  referralCount_.Text(hstring{std::to_wstring(total) + L"/" + std::to_wstring(terms.maxReferrals)});
