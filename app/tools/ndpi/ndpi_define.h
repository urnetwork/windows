 ...
 * (Phase 0 gate, task-13).
 *
 * Upstream generates this file at configure time from
 * src/include/ndpi_define.h.in via AC_CONFIG_FILES -- a step that does not
 * run when cross-compiling with the vendored Makefile.crossproof (no
 * autotools involved), so it is checked in here pre-expanded instead.
 *
 * Source: ntop/nDPI tag 5.0 (commit 375f99ef9fb4999d778b57bbeece171b3fa9fba6),
 * file src/include/ndpi_define.h.in, with the same @VAR@ substitutions
 * ./configure would perform for a library-only build:
 *   @NDPI_API_VERSION@ -> 0   (upstream's own windows/src/ndpi_define.h,
 *                              the checked-in MSVC copy, uses the same 0 --
 *                              "Not supported for Visual Studio.")
 *   @NDPI_MAJOR@ -> 5, @NDPI_MINOR@ -> 0, @NDPI_PATCH@ -> 0
 *                              (from AC_INIT([libndpi],[5.0.0]) at this tag)
 ...
 * this file correctly carries NDPI_MAJOR=5 (the MSVC copy is stale at 0,
 * which upstream itself never fixed) and the __APPLE__/_WINDOWS_
 * htobe/htole helper block's idempotency (#ifndef) guards, neither of which
 * changes behavior for this build (WIN32/_WINDOWS_ only, not __APPLE__).
 */
