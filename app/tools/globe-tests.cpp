...
void GlobeGeometryTests() {
  ...
  }
  {
    ...
    TEST_CASE("theWheelWrapsAtBothEnds");
    CheckEqualInt(0, GlobeGeometry::WrapIndex(2, 1, 3), "wrap forward");
    CheckEqualInt(2, GlobeGeometry::WrapIndex(0, -1, 3), "wrap backward");
    CheckEqualInt(1, GlobeGeometry::WrapIndex(0, 1, 3), "step forward");
    // a multi-step drag wraps more than once
    CheckEqualInt(1, GlobeGeometry::WrapIndex(0, 7, 3), "multi wrap forward");
    CheckEqualInt(2, GlobeGeometry::WrapIndex(0, -7, 3), "multi wrap backward");
  }
  {
    TEST_CASE("wrapIndexReportsNoIndexForAnEmptyWheel");
    CheckEqualInt(-1, GlobeGeometry::WrapIndex(0, 1, 0), "empty wheel");
  }
...
void ProviderLocationsTests() {
  ...
  {
    TEST_CASE("wheelOrderIsWestToEastAndDropsUnplottableRows");
    std::vector<ProviderLocationRow> rows(4);
    rows[0].clientId = "a";
    rows[0].hasCoordinates = true;
    rows[0].lon = 10;
    rows[1].clientId = "b";  // no coordinates: listed but not on the globe
    rows[2].clientId = "c";
    rows[2].hasCoordinates = true;
    rows[2].lon = -120;
    rows[3].clientId = "d";
    rows[3].hasCoordinates = true;
    rows[3].lon = 140;

    const auto wheel = WheelOrder(rows);
    CheckEqualInt(3, static_cast<long long>(wheel.size()), "wheel size");
    CheckEqualString("c", wheel[0].clientId, "westernmost");
    CheckEqualString("a", wheel[1].clientId, "middle");
    CheckEqualString("d", wheel[2].clientId, "easternmost");
    CheckEqualInt(0, static_cast<long long>(WheelOrder({}).size()), "empty input");
  }
}
